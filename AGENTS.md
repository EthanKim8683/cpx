...
> these steps, check whether configs already exist:
>
> ```bash
> test -f internal/gcc/generated_cdbconfig.go && test -f internal/clang/generated_cdbconfig.go && echo "already bootstrapped"
> ```
>
> If they do, skip bootstrapping.
...
1. Read `.env.example` and confirm `GO`, `PYTHON3`, `GCC`, `GXX`, `CLANG`, and `CLANGXX` paths with the user.
   Verify each path exists. If the user doesn't have GCC or Clang installed,
   offer to help them install it (e.g. `brew install gcc` or `brew install llvm`).
