...
func ExecuteGCC(cfg *config.Config, args []string, dir string) error {
	bin := cfg.GCC
	if bin == "" {
	...
	shim := &cdb.Shim{
		...
		Store:    cdb.NewFileStore(cfg.CDB),
	}
	return shim.Execute(args, dir)
}

// ExecuteGXX executes the g++ driver shim under the given configuration.
func ExecuteGXX(cfg *config.Config, args []string, dir string) error {
	bin := cfg.GXX
	if bin == "" {
